...
func isValidCollectionKey(key any) error {
	switch t := key.(type) {
	case string:
		// make sure it's not reserved and it's not empty/whitespace-only
		if t == CMWC_T {
			return fmt.Errorf("bad collection key: %s is reserved", CMWC_T)
		}
...
func isValidCollectionType(ctyp string) error {
	// "__cmwc_t": ~uri / oid
...
func (o *collection) addItem(key any, node *CMW) error {
	if err := isValidCollectionKey(key); err != nil {
...
func (o *collection) UnmarshalCBOR(b []byte) error {
	...
	cmwc_t, found := tmp[CMWC_T]
	if found {
		var s string
		if err := dm.Unmarshal(cmwc_t, &s); err != nil {
			return fmt.Errorf("extracting CBOR collection type: %w", err)
		}
		if err := isValidCollectionType(s); err != nil {
...
func (o *collection) UnmarshalJSON(b []byte) error {
	...
	if found {
		var s string
		if err := json.Unmarshal(cmwc_t, &s); err != nil {
