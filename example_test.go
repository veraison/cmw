...
func makeCMWCollection() *CMW {
	sub := NewCollection(("tag:ietf.org,2024:Y"))

	node := NewMonad("application/eat-ucs+json", []byte(`{"eat_nonce": ...}`), AttestationResults)
	_ = sub.AddCollectionItem("polyscopic", node)

	root := NewCollection("tag:ietf.org,2024:X")

	_ = root.AddCollectionItem("murmurless", sub)

	node = NewMonad("application/eat-ucs+cbor", []byte{0xa1, 0x0a})
	_ = root.AddCollectionItem("bretwaldadom", node)

	node = NewMonad("application/eat-ucs+cbor", []byte{0x82, 0x78, 0x18}, ReferenceValues, Endorsements)
