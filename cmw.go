...
func NewMonad(mediaType any, value []byte, indicators ...Indicator) *CMW {
	var c CMW
	c.val.Set(value)
	c.typ.Set(mediaType)
...
func NewCollection(cmwct string) *CMW {
	if cmwct != "" && isValidCollectionType(cmwct) != nil {
		return nil
	}
	var c CMW
...
// Obtain (sorted) keys and types in a collection
...
func (o CMW) MarshalCBOR() ([]byte, error) {
	if len(o.cmap) == 0 { // record or CBOR tag
