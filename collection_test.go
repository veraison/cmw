...
func Test_Collection_JSON_Deserialize_ok(t *testing.T) {
	...
	assert.Equal(t, "application/vnd.a", a.GetMonadType())
	assert.Equal(t, []byte{0x61}, a.GetMonadValue())
	assert.Equal(t, Indicator(0), a.GetMonadIndicator())

	b, err := actual.GetCollectionItem("b")
	assert.NoError(t, err)
	assert.Equal(t, FormatJSONRecord, b.GetFormat())
	assert.Equal(t, KindMonad, b.GetKind())
	assert.Equal(t, "application/vnd.b", b.GetMonadType())
	assert.Equal(t, []byte{0x62}, b.GetMonadValue())
	assert.Equal(t, Indicator(0), b.GetMonadIndicator())
}
...
func Test_Collection_JSON_Serialize_ok(t *testing.T) {
	...
	a := NewMonad("application/vnd.a", []byte{0x61})

	err := tv.AddCollectionItem("a", a)
	require.NoError(t, err)

	b := NewMonad("application/vnd.b", []byte{0x62})
...
func Test_Collection_CBOR_Deserialize_ok(t *testing.T) {
	...
	assert.Equal(t, "application/signed-corim+cbor", one.GetMonadType())
	assert.Equal(t, []byte{0xd2, 0x84, 0x43, 0xa1, 0x1, 0x26, 0xa1}, one.GetMonadValue())
	assert.Equal(t, Indicator(3), one.GetMonadIndicator())

	two, err := actual.GetCollectionItem(uint64(2))
	assert.NoError(t, err)
	assert.Equal(t, KindMonad, two.GetKind())
	assert.Equal(t, FormatCBORTag, two.GetFormat())
	assert.Equal(t, "29884", two.GetMonadType()) // TN() mapped CoAP C-F
	assert.Equal(t, []byte{0x23, 0x47, 0xda, 0x55}, two.GetMonadValue())
	assert.Equal(t, Indicator(0), two.GetMonadIndicator())

	s, err := actual.GetCollectionItem("s")
	assert.NoError(t, err)
	assert.Equal(t, KindMonad, s.GetKind())
	assert.Equal(t, FormatCBORRecord, s.GetFormat())
	assert.Equal(t, "30001", s.GetMonadType())
	assert.Equal(t, []byte{0x23, 0x47, 0xda, 0x55}, s.GetMonadValue())
	assert.Equal(t, Indicator(0), s.GetMonadIndicator())
}

func Test_Collection_CBOR_Serialize_ok(t *testing.T) {
	expected := mustReadFile(t, "testdata/collection-cbor-ok-2.cbor")

	item1 := NewMonad("application/vnd.1", []byte{0xde, 0xad, 0xbe, 0xef})
...
func Test_isValidCollectionType(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := isValidCollectionType(tt.args.ctyp); (err != nil) != tt.wantErr {
...
func Test_isValidCollectionKey(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := isValidCollectionKey(tt.args.key); (err != nil) != tt.wantErr {
